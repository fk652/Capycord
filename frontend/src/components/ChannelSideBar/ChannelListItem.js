...
const ChannelListItem = ({id, name, type, selected}) => {
  ...
  if (selected && serverInfo && !channelSettingsId) document.title = `Capycord | #${name} | ${serverInfo.name}`
  ...
  let icon = null;
  const textIcon = (
    <svg width="20" height="20" viewBox="0 0 24 24" background="background-2neGeL">
  ...
  return (
    <div 
      ...
      onClick={() => history.push(`/server/${serverId}/${id}`)}
    >
